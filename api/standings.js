...
// Fetch OpenF1 Auth Token securely
...
export default async function handler(req, res) {
  ...
  res.setHeader('Cache-Control', 's-maxage=3600'); // Cache main dashboard for 1 hr

  console.log(`[STANDINGS] Starting dashboard data aggregation...`);

  try {
    // 1. Authenticate with OpenF1 for Weather/Stints
    ...
    // DECOUPLED QUALIFYING: Fixes the Monaco vs Canada label mismatch
    let qualifying = null;
    const qualiRace = getRaces(qualiData)[0];
    if (qualiRace && qualiRace.QualifyingResults) {
      qualifying = {
        short_name: (qualiRace.raceName || '').replace(' Grand Prix', ''),
        pole: {
    ...
    // DECOUPLED WEATHER: Maps to whatever session OpenF1 is currently looking at
    let weather = null;
    if (openf1Weather && Array.isArray(openf1Weather) && openf1Weather.length > 0) {
      const w = openf1Weather[openf1Weather.length - 1];
      const s = (openf1Session && openf1Session.length > 0) ? openf1Session[0] : null;
      weather = {
    ...
    res.status(200).json({ drivers, constructors, schedule, next_race, last_race, qualifying, weather, winners, _fetched_at_utc: new Date().toISOString() });
  } catch (error) {
